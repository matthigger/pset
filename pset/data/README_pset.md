...
You do not define them anywhere: `pset.tex` at the repo root does, and it
is generated by `pset --init` rather than written by hand, because a
mistake in those definitions prints the answers on the student copy.  The
shared preamble inputs it and then styles the result:

    \input{../pset.tex}
    \renewcommand{\solstyle}[1]{...}

So `pset.tex` is not yours to edit -- `pset --init` overwrites it to
upgrade.  Restyle through `\solstyle`, `\rubstyle` and `\probstyle`
instead, in the preamble, after the input.
